...
public class BsDailyInOutCB extends AbstractConditionBean {
     ...
     * </pre>
     */
    public void setupSelect_VacationType() {
        assertSetupSelectPurpose("vacationType");
        if (hasSpecifiedLocalColumn()) {
            specify().columnVacationTypeCode();
        }
        doSetupSelect(() -> query().queryVacationType());
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<DailyInOutCQ> {
        ...
        protected void doSpecifyRequiredColumn() {
            ...
            if (qyCall().qy().hasConditionQueryVacationType()
                ...
                columnVacationTypeCode(); // FK or one-to-one referrer
            }
        }
        @Override
        ...
        public VacationTypeCB.HpSpecification specifyVacationType() {
            assertRelation("vacationType");
            if (_vacationType == null) {
                _vacationType = new VacationTypeCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryVacationType()
                                    , () -> _qyCall.qy().queryVacationType())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _vacationType.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryVacationType()
                      , () -> xsyncQyCall().qy().queryVacationType()));
                }
            }
