...
schemaDef.get('properties').map((v, k) => {
  if (k !== 'error') {
    ...
    const packageName = config.outputDir.substring(config.outputDir.indexOf(config.packageRootDir) + config.packageRootDir.length).replace(/\//g, '.')
    const kotlinClass = `// Code generated by Node.js script\npackage ${packageName}\n\n${generateKotlinDataClass(k, v, 0)}\n`
    fs.writeFileSync(`${config.outputDir}/${k}.kt`, kotlinClass)
    console.log('')
  }
})
