    ...
    return new Promise((resolve, reject) => {
      $RefParser.dereference(`${schemaDir}/${fileName}`, (err, schema) => {
        ...
        const generate = (path, method, responsesObject, status) => {
          ...
          fs.writeFileSync(`${config.sampleJsonDir}/${schemaName}.json`, JSON.stringify(json, null, 2))
          // Kotlinクラス作成
          ...
          fs.writeFileSync(`${testClassFileDir}/${schemaName}Test.kt`, generateKotlinTestClass(schemaName, schema))
        }
        properties.get('paths').map((pathsObject, path) => {
          pathsObject.map((pathsItem, method) => {
            pathsItem.get('parameters').filter(v => v.get('in') === 'body').map(v => generate(path, method, v, 'request'))
            pathsItem.get('responses').map((v, k) => generate(path, method, v, k))
          })
        })
